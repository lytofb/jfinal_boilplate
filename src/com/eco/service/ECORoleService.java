...
public class ECORoleService implements RoleInterface {
	...
	public List<Roles> getRoles() {
		...
		for(int i = 0 ; i < results.size() ; i ++){
			...
			role.set("id", results.get(i).getInt("id"));
			role.set("role_nickname", results.get(i).getStr("rolename"));
			role.set("role_name", "#");
			roles.add(role);
		}
		return roles;
	...
	public void addUserRoleRela(Integer user_info_id,Integer role_info_id) {
		// TODO Auto-generated method stub
	Record user_role = new Record().set("user_info_id", user_info_id).set("role_info_id", role_info_id);
	Db.save("user_role_rel", user_role);
		 ...
		 String result ="" ;
		   if(checkRoleName(roleName)==true){
			   if(checkRoleNick(roleNick)==true){
				   ...
				   result = Integer.toString(role.getInt("id"));
			   }else{
