...
(function (factory) {
	...
	Tree.prototype = {
		...
		recursiveRender:function recursiveRender(originalData,branchTemplate,itemTemplate,$el,subnodeFlag){
			var $parent = ($el.hasClass('tree')) ? $el : $el.parent();
			var self = this;
			$.each(originalData,function (index,value){
				var $entity;
				branchTemplate_self = branchTemplate.clone(true,true);
				itemTemplate_self = itemTemplate.clone(true,true);
				if(value.type === 'folder') {
					$entity = branchTemplate_self;
					$entity.data(value);
					$entity.find('.tree-branch-name > .tree-label').html(value.text || value.name);
				} else if (value.type === 'item') {
					$entity = itemTemplate_self;
				...
				$.each(attr, function (key, value) {
					switch (key) {
						...
						// style, data-*
						case 'subnode':
							$entity = self.recursiveRender(value,branchTemplate.clone(true,true),itemTemplate.clone(true,true),$entity,true)
							var $branch = $entity.closest('.tree-branch');
				...
				if($el){
			          if ($el.hasClass('tree-branch-header')) {
			            ...
			            } else if(subnodeFlag&&$el.hasClass('tree-branch')) {
			              $el.find('.tree-branch-children:eq(0)').append($entity);
			            } else {
			              $el.append($entity);
			            }
				}
		...
		populate: function populate($el) {
			...
			this.options.dataSource(treeData ? treeData : {}, function (items) {
				...
				var branchTemplate = self.$element.find('[data-template=treebranch]:eq(0)').clone(true,true).removeClass('hide hidden').removeData('template'); // hide is deprecated
				var itemTemplate = self.$element.find('[data-template=treeitem]:eq(0)').clone(true,true).removeClass('hide hidden').removeData('template'); // hide is deprecated
				var $entity = self.recursiveRender(items.data,branchTemplate,itemTemplate,$el)
